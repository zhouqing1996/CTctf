<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb-css">
      ...
      <el-breadcrumb-item><span >更新多选题</span></el-breadcrumb-item>
    </el-breadcrumb>
    <div><hr/></div>
    <div>
      <el-row :gutter="20">
        ...
        <el-col :span="6">
          <span>
            <el-button v-on:click="changeChooseM" type="success" >更新该题目</el-button>
            <br>
            <br>
...
<script>
  export default {
    ...
    methods:{
      ...
      {
        let that = this
        that.$http.post('/bank/choosem/info',{
          id:that.addList.id
        }).then(function (res) {
          console.log(res.data)
          let info = res.data.data
          that.addList.rem=info['mqrem']
          that.addList.tail=info['mqtail']
          that.addList.ans=info['mqans']
          that.addList.op1=info['mqcho'].split('---')[0]
          that.addList.op2=info['mqcho'].split('---')[1]
          that.addList.op3=info['mqcho'].split('---')[2]
          that.addList.op4=info['mqcho'].split('---')[3]
          that.addList.item=info['mqitem']
        })
      },
      //返回
      back:function()
      ...
      changeChooseM:function () {
        ...
        if(that.addList.item == '')
        {
          ...
          that.$http.post('/bank/choosem/changechoosem',{
            id:that.addList.id,
