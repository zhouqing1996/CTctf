        ...
        <!--<el-tab-pane>-->
          ...
          <div class="display2">
            <div class="searchmem">
              ...
              <button class="btn el-icon-circle-plus-outline" v-on:click="addProgram">添加</button>
              ...
              <button class="btn el-icon-folder" @click="getQueryPY">有效题目</button>
              <button class="btn el-icon-folder-remove" @click="getQueryPN">无效题目</button>
              <button class="btn el-icon-folder-checked" @click="getQueryP">所有题目</button>
              <button class="btn el-icon-document" @click="addF">批量添加</button>
              <button class="btn el-icon-document" @click="exportP">导出</button>
            ...
            <div class="waimian">
              <div class="detail" v-for=" (P,key) in currentPageData">
                <div>
                  <div>
                    <h3>
                      ...
                      <div class="delete">
                        <span v-if="P.pqstatus==1" class="valid_span">有效
                        <span @click="deleteprogram(1,P.pqid)" class="span1"><i class="el-icon-delete">删除</i></span>
                        </span>
                        <span v-if="P.pqstatus==0" class="novalid_span">无效
                        ...
                        <span v-on:click="change(P.pqid)" class="span3">修改题目</span>
                      </div>
                    </h3>
    ...
    methods:{
      ...
      addP:function (List) {
        ...
        if(List.item == '')
        {
          ...
          that.$http.post('/bank/program/addprogram',{
            qitem:List.item,
            ans:List.ans,
            tail:List.tail,
            rem:List.rem,
      ...
      changeprogram:function (item,id) {
        ...
        console.log(item)
        if(item==1) {
          that.$http.post('/bank/program/change',{
      ...
      deleteprogram:function (item,id) {
        ...
        if(item==1)
        {
          that.$confirm("删除该用户程序题，是否继续？", "提示", {
            ...
            that.$http.post('/bank/program/delete',{
              fid:id,
              flag:1,
              auth:that.$store.getters.getsId
            }).then(function (res) {
      ...
      importExcel (obj) {
        ...
        FileReader.prototype.readAsBinaryString = function (f) {
          ...
          reader.onload = function (e) {
            ...
            console.log(data)
            _that.$http.post('/bank/program/importexcel', data).then(body => {
